...
def calculate_E_neo(P_elec, F_elec, Hcore_elec, P_prot, F_prot, Hcore_prot) -> float:
    ...
    for i in range(N_PROT_BASIS):
        for j in range(N_PROT_BASIS):
            E += P_prot[i, j]*(Hcore_prot[i, j] + F_prot[i, j])/2
    return E
...
def scf(coeffs):
    alpha_elec, beta_elec, alpha_prot, beta_prot = np.exp(coeffs)
    verbose = False
    elec_basis = [Gaussian_basis(COORDS[0], "optimize", d1=1, a1=alpha_elec*beta_elec**i) for i in range(6)]
    prot_basis = [Gaussian_basis(COORDS[1], "optimize", d1=1, a1=alpha_elec*beta_elec**i) for i in range(6)]
    
    global N_ELEC_BASIS
    global N_PROT_BASIS
    N_ELEC_BASIS = len(elec_basis)
    N_PROT_BASIS = len(prot_basis)
    prot_mass = 1
    ...
    F_elec = H_elec + J_elec - K_elec - mixed_elec
    ...
    while E_diff > tol:
        ...
        while norm_p > tol_p:
            ...
            F_prot = H_prot + J_prot - K_prot - mixed_prot
            F_prot = H_prot - mixed_prot                  # THESE TWO ARE INVARIANT
            F_prot_tr = X_prot.T@F_prot@X_prot
        ...
        E_old = E_new
    print(f"Alpha_elec = {alpha_elec:.8f}, Beta_elec = {beta_elec:.8f}, Alpha_prot = {alpha_prot:.8f}, Beta_prot = {beta_prot:.8f}, Total energy is {E_new}")
    return E_new


coeffs = (3, 0.1, 10, 0.3) # -0.10841701620255256
coeffs = np.log([3, 0.3, 3, 0.3])  # -0.10851258388786147

# bounds = ((0, 100), (0, 100), (0,100), (0,100))

def different_alphas(t):
    return t[1]!=t[3]

cons = [{'type':'eq', 'fun': different_alphas}]


min_coeffs = minimize(scf, coeffs)#, bounds=bounds)
